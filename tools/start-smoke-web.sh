...
cd "$WEB_DIR"
sh "$REPO_ROOT/tools/ensure-node-app-deps.sh" "$WEB_DIR" 'node -e "const fs = require(\"fs\"); fs.statSync(\"node_modules/.bin/vite\"); fs.statSync(\"node_modules/@robertvejvoda/fairspot-api-client/package.json\"); fs.statSync(\"node_modules/@robertvejvoda/fairspot-ui/package.json\")"'
