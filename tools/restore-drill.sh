...
#
#   MongoDB            mongorestore --archive --gzip --drop --oplogReplay
...
#   Vault              raft snapshot restore is a NAS/server-mode operation and
#                      is intentionally NOT auto-applied to a -dev local Vault
#                      (dev Vault holds no durable secrets). Printed as guidance.
#
...
#
# Exit codes: 0 restore + smoke + data-return all passed; 1 otherwise.
...
if _has "$VAULT_SNAPSHOT_ARTIFACT" || _has "$VAULT_TAR_ARTIFACT"; then
  warn "Vault backup present. Vault restore is a NAS/server-mode operation and is NOT"
  warn "auto-applied to the local dev Vault. On NAS, after init+unseal, run:"
  warn "    vault operator raft snapshot restore $VAULT_SNAPSHOT_ARTIFACT"
...
ok "Restore drill PASSED — stack rebuilt from $FROM, smoke green, data returned"
