...
In the **Green Logistics** showcase, sign in as a general employee (for example `gl-employee5`, password `Dev1234!`) and submit a `GL-HQ` parking request, then watch the Draw place you as allocated or waitlisted. `gl-employee1` holds a company-car slot (`VIP-01`) and is allocated before the fair Draw — showing how obligations differ from lottery preference. Full steps: [Green Logistics Walkthrough](./green-logistics-walkthrough).
