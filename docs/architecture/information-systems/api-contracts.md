...
| HR operations APIs | Booking / DataHub | Booking API contract and future DataHub query contracts. | Placeholder | Tenant/location-scoped request queues, next Draw status, controlled Draw, and cancellation with reason. |
...
| Configuration APIs | Configuration | OpenAPI/generated TypeScript client | Partial | Policy/location/slot/resource-map admin surfaces and effective version history. |
...
| DataHub APIs | DataHub | Future query contracts | Placeholder | Projection ownership, privacy shape, query filters, pagination, and export boundaries must be explicit. |
