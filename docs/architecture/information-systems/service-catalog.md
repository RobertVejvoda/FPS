...
| Configuration | Tenant parking policy, location, time slot, capacity/resource map, and Draw schedule. | Policies, locations, slots/resources, capacity history. | Admin APIs, policy/capacity events. | High | Partial |
...
| DataHub | Cross-service read models and projection health. | Event inbox, projections, checkpoints, read stores. | Query APIs, dashboard/report views, export data. | Medium/High | Placeholder |
