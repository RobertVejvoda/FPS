...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GAP-001 | Persistence | Current State v0.1 | Customer-Ready Target v0.1 | Customer service has in-memory repositories for some tenant state. | All service-owned write stores are durable, tenant-scoped, and backed by provider-neutral Dapr components. | Customer primary state is Dapr-backed (`DaprCustomerTenantRepository`, `DaprCustomerIdentityRepository`, `DaprCustomerParkingBootstrapRepository`); in-memory stores are restart-safe hydration caches. Configuration, Profile, Audit, Notification, and Booking fairness metrics remain in-memory — production-blocking for customer traffic. DataHub is durable (EF Core/PostgreSQL) but projection rebuild path is not documented. | In-memory stores for Configuration, Profile, Audit, Notification, and Booking fairness mean tenant data is lost on service restart. Booking eligibility, fairness draws, audit compliance, and user inbox are all affected. | OPS008 — persistence profile and implementation sequence (PERSIST001–PERSIST006). See [docs/production/ops008-persistence-profile.md](../../production/ops008-persistence-profile.md). | Claude/Codex | Planned |
