...
| Slice | Goal | Notes |
| --- | --- | --- |
| `CUST002` Customer Data Import Contract | Define how employee IDs, names, emails, roles, active status, cars, company-car eligibility, accessibility flags, and location mapping enter FPS. | Must include Confidential/Secret classification and source-of-truth rules. |
| `P002` Profile Import And Mapping | Implement the first import path for profile facts. | CSV/import-file path is the likely first step before HR API integration. |
| `ID002` User Provisioning Integration | Map imported users to Identity claims, roles, and deactivation behavior. | Could later use IdP claims or SCIM. |
| `OPS005` Integration Secrets And Observability | Define integration credentials, audit, metrics, retries, and error evidence. | Required before customer-owned production integrations. |
