...
| `Module Reservations UX and Seats` | Future `UX`/`SEAT` slices | Remove duplicated parking/seat navigation and prove Seats as a tenant-enabled module using the same fair reservation foundation. | Employees use one date-grouped My Reservations page and one date-first Request page; Seats supports team-area priority, specific/alternate seat preferences, simple grid map, recurring request rules, and tenant-configured usage confirmation. | This is the UX and product bridge from the parking proof path to FairSpot as a multi-module scarce-capacity platform. |
