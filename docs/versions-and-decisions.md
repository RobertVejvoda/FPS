...
| **MOB003 mobile login scope** | The third mobile slice replaces development bearer-token handoff with OIDC Authorization Code + PKCE in the Expo app. The app validates sessions with `GET /me`, stores no client secret, and never supplies tenant/user/role identity for employee API scoping. Keycloak provisioning, booking actions, notification streaming, native packaging, and backend business changes remain out of scope. | Architect | 13.5.2026
| **Security data classification** | FPS classifies data as Public, Internal, Confidential, or Secret. Confidential tenant/employee data requires authenticated tenant-scoped authorization, encryption in transit and at rest, and audit for sensitive administrative access. Secret data is limited to credentials, keys, tokens, certificates, connection strings, and recovery material; it must be stored in secret-management systems, never committed or logged, and human access must be justified, time-bound, audited, and followed by rotation when exposed. | Architect | 14.5.2026
| **SSO-first customer integration** | Company users authenticate through the customer's identity provider by default. FPS stores only the minimum mapped subject, tenant, role, and policy facts required for booking, notification, audit, reporting, and support. FPS does not store company passwords. FPS-local accounts are fallback accounts; their credential verifiers are Secret data and must use hardened Identity storage. CSV/file import is bootstrap or exception only, never a password or broad HR-data import path. | Architect | 17.5.2026
| **OPS001 Pluggable Dapr component baseline** | Dapr components are organized by deployment profile: local (Docker Compose), demo (managed services), and production examples (Azure/AWS/GCP/K8s). Component boundaries for pub/sub, state, secrets, bindings, identity, and observability are documented. Collection-per-tenant provisioning guidance, secret-store pattern, and component replacement runbook are defined. Application code remains behind Dapr APIs; swapping components requires YAML changes and validation, not code rewrites. | Architect | 17.5.2026
