...
| **Public documentation domain** | The public FairSpot documentation site uses `fairspot.net` as its custom GitHub Pages domain. `docs/CNAME` is part of the deployment artifact and must remain in `docs/` because the Pages workflow publishes that directory directly. Existing `www.vejvoda.net/FPS/` links may remain as compatibility links while public material moves to the FairSpot domain. | Codex/Product Owner | 24.5.2026
