...
> **Release 1 producer reconciliation (DATAHUB004 / #335).** The authoritative Booking producer event names emitted by `BookingDaprEventPublisher` are `booking.requestSubmitted`, `booking.requestRejected`, `booking.requestCancelled`, `booking.slotAllocated`, `booking.usageConfirmed`, and `booking.noShowRecorded`. The DataHub projection handler also accepts the legacy alias `booking.requestAllocated` for `booking.slotAllocated`. `booking.requestExpired` is projection-supported (the handler maps it to a terminal `Expired` outcome) but is **not currently emitted** by Booking in Release 1 — no expiry-event publisher exists yet, so it is retained here as a forward contract. Booking also emits `booking.drawStarted` / `booking.drawCompleted` / `booking.drawFailed` (projected into Draw history) and `booking.penaltyApplied` / `booking.manualCorrectionApplied` (not projected into outcome read models in Release 1).
...
| `booking_outcome_summary` | DataHub | `booking.requestSubmitted`, `booking.requestRejected`, `booking.slotAllocated`, `booking.requestCancelled`, `booking.usageConfirmed`, `booking.noShowRecorded`, `booking.requestExpired`. | My Spots summaries, HR queue counts, operational demand and lifecycle summaries. | Does not replace Booking `GET /bookings` authoritative own-booking queries until explicitly migrated. No raw lottery internals. |
