...
| Area | Decision |
| --- | --- |
| Product posture | Start small, open-source, scarce shared-capacity-first, B2B, and pilot-led, with parking as the first launch module. |
| Target customer | Small B2B organizations, initially below about 150 active users, with scarce shared capacity and visible allocation friction. Parking remains the first concrete buying trigger. |
| Core story | Fair allocation, low operator administration, transparent outcomes, client-owned data, and privacy-conscious user records. |
...
- **Open core** — the fairness/Draw engine, audit/evidence, standard resource booking flows, and the user- and tenant-facing application. These stay open.
...
The free/open core must be useful enough for a B2B organization to evaluate and run a normal tenant. Do not make the fairness story depend on paid unlocks.

| Capability | Free/open core expectation | Why it stays open |
| --- | --- | --- |
| Tenant setup | Standard tenant creation, identity setup, first administrator, policies, resources/capacity, and readiness checks. | An organization must be able to prove FairSpot can operate before discussing commercial services. |
| User workflow | Login, request a resource booking, view bookings, cancel, confirm usage, receive operational notifications, and see safe allocation outcomes. | User trust is the product, not an add-on. |
| Fair allocation | Draw, same-day allocation, penalties, reallocation, reason codes, and user-safe explanations. | Allocation logic must remain inspectable to be trusted. |
| Audit and privacy | Tenant-scoped audit, pseudonymised records, erasure support, and minimal user facts. | Data ownership and privacy are core objections during evaluation. |
...
| Support subscription | Response targets, release guidance, upgrade help, security advisory handling, and operational assistance. | Should be contract-level, not tied to user booking behavior. |
...
- Do not make basic tenant operation, booking, or user trust screens paid-only.
- Do not make privacy controls or data export/removal evidence paid-only.
- Do not couple cost recovery to individual user allocation outcomes by default.
- Do not introduce broad user personal data into commercial records.
...
- which data is required for commercial records and which user data is explicitly excluded;
...
- FairSpot should prefer customer identity-provider subjects and tenant-scoped pseudonymous records over names, employee IDs, member IDs, or broad people profiles.
- User booking data is for resource operations, audit, reporting, and fair allocation. It should not become commercial input without a separate approved decision.
- Contract-level commercial contacts, if later needed, should be tenant-scoped and separate from user booking records.
- Client-owned deployment remains a strong default for organizations that treat user and resource-access data as sensitive.
...
| Step | Evidence |
| --- | --- |
| Pilot with one or two small organizations | Can the tenant be onboarded, tested, and operated with limited support? |
| Measure admin reduction | Compare email/spreadsheet effort before and after FairSpot. |
| Measure fairness trust | Track complaints, manual overrides, user questions, and explanation gaps. |
