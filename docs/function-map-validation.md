...
The map is broadly directionally correct, but several boxes are either not current app priorities or are implemented under different boundaries. The former Customer persistence gap has been addressed with Dapr-backed runtime repositories. DataHub now owns the PostgreSQL-backed durable projection direction; Reporting remains a transitional report surface with an in-memory compatibility repository and should not receive new projection storage by default. Billing and Payment Gateway are valid target/reference capabilities but not a priority for making the app work. Feedback is not implemented, but a small authenticated feedback path would be reasonable for testing and customer evaluation.
...
| Customer | Customer Services. | Customer manages tenant lifecycle, identity setup, parking bootstrap, first admins, readiness, tenant requests, and future support/customer service. Tenant storage contract requires Customer tenant registry, onboarding state, and identity configuration to be physically stored. | `FPS.Customer` service exists with tenant, identity, parking bootstrap, readiness, tenant request, and sandbox reset APIs. Runtime wiring uses Dapr-backed Customer repositories; in-memory repositories remain for tests. | Implemented for current baseline; hosted restore/smoke evidence remains required before real customer data. |
...
| Reporting | Reporting. | Reporting consumes Booking outcomes and exposes dashboard, fairness, utilization, reason-code, and exports. DataHub owns durable cross-service projections. | `FPS.Reporting` service and web Reporting page exist, but Reporting currently registers `InMemoryReportingRepository`; DataHub has PostgreSQL-backed projection storage. | Partial/transitional. Keep Reporting as report surface or compatibility API; put durable projection work in DataHub. |
...
| Relational Data Store | Data Management. | Deployment can choose operational/document stores behind Dapr boundaries; DataHub uses PostgreSQL for cross-service read models. | Current local profile uses MongoDB through Dapr state stores plus DataHub PostgreSQL. | Implemented for DataHub projections. Do not make Reporting the owner of new PostgreSQL operational projections. |
...
- [docs/business-layer/functional-architecture.md](./business-layer/functional-architecture.md) has been narrowed toward the current product baseline. Keep future-only UI, feedback, support, billing, SMS/push, and device-capability items explicitly marked as deferred when they remain as target context.
...
- Customer Service durable persistence is no longer the main gap; keep hosted restore/smoke evidence current.
...
1. Current baseline: Web App, Mobile App, Booking, Profile, Configuration, Identity, Notification, Audit, Customer, DataHub first projections, Reporting endpoints, Event Bus, API Gateway, Dapr, MongoDB-backed Dapr state stores, PostgreSQL-backed DataHub, observability.
2. Partial/current-boundary items: Reporting/DataHub read-path reconciliation, Configuration-to-Booking runtime exchange, Dapr Workflows, File/Object Storage, Secure Storage, Communication, Support/HR.
...
- Keep Customer durable storage and DataHub projection evidence linked from release/readiness docs.
- Reconcile DataHub durable projections with any Reporting compatibility endpoints. Keep Reporting limited to report catalog/configuration or compatibility APIs.
