...
| Gap | Severity | Notes |
|-----|----------|-------|
| Hosted encryption gate not evidenced | High — production-blocking; **automated gate built (SEC011 #616)** | NAS and later hosted profiles must prove HTTPS public URLs, Cloudflare/WAF boundary, blocked internal/admin endpoints, and no accidental public exposure of Dapr, stores, observability, or management ports. `tools/smoke-hosted.sh` now evidences these automatically (TLS/redirect [#9]; `/metrics`, admin, `/api/*` incl. Dapr invoke & metrics, and infra-hostname exposure [#10]); the public readiness expectation is summarized in [hosted-smoke-runbook.md](../production/hosted-smoke-runbook.md#public-readiness-expectations), while the detailed operator-confirm checklist lives in the private `fairspot-platform` runbook. Closed against the public domain when run there with all mandatory checks PASS. |
...
| Dapr mTLS/service identity not enabled for hosted profiles | High — production-blocking | Dapr mTLS is disabled in local config (`fairspot-config.yaml`). Local remains exempt, but NAS/client profiles must enable and evidence Dapr mTLS or document an approved equivalent service-to-service encryption boundary. |
...
| Gap | Severity | Notes |
|-----|----------|-------|
| Hosted secret store still uses transitional Vault development mode | **Resolved (Phase 1) — #630 / OPS018 #618** | NAS now runs Vault in durable **server mode** (raft persistence, manual unseal, least-privilege `fairspot-dapr` token sourced from `nas.env`, no reusable dev root token); see [nas-cloudflare-deployment-profile.md](../production/nas-cloudflare-deployment-profile.md) Step 6a. Local keeps dev mode. **Phase 2 started — DataHub/Postgres (SEC012A #742):** the committed `Password=fps` static-production fallback is removed — base `appsettings.json` ships no connection string, so DataHub fails closed unless `ConnectionStrings__DataHub` is injected (local dev keeps a dev-only `appsettings.Development.json`). The NAS dynamic-secret target (Vault database secrets engine + Vault Agent) and why Dapr Vault KV cannot carry a leased DB credential are documented in [dapr-first-production-standards.md](../production/dapr-first-production-standards.md#dynamic-database-secrets-sec012a). **Remaining (Phase 2):** MongoDB, RabbitMQ, and MinIO dynamic secrets — tracked in #628. |
...
| Gap | Severity | Notes |
|-----|----------|-------|
| Employee data erasure workflow needs durable store completion | High — production-blocking | PRIV001 is implemented: authorised privacy/admin users can create an erasure request; Dapr Workflow coordinates service-owned steps; notification deletion, Audit PII mapping deletion, per-step audit records, and APP_API_TOKEN-protected internal erasure endpoints are in place. **Reporting anonymisation (DataHub, #763 → #772 resolved):** since Reporting reads durable report data from DataHub's projections, its in-memory anonymisation is a no-op in Production; DataHub now owns a user-level erasure endpoint (`[DaprInternalOnly] POST /erasure`, #772) that re-points a single subject's `RequestorId` in the durable outcome projection to an irreversible pseudonym, and the PRIV001 reporting-anonymisation step calls it — so DataHub-backed reports are now anonymised for a subject. Production remains blocked until Profile erasure and Booking active-check/anonymisation move beyond local/durable-store stubs and have smoke evidence against the selected production store. |
