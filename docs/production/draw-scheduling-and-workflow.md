...
| --- | --- | --- |
| `DRAW006` Simulation-driven scheduled Draw | Make non-production time travel actually run due scheduled Draws for the configured tenant/location/time slot. **Status: Implemented.** | Advancing simulation time across the Draw time triggers the same workflow as the Dapr cron/manual trigger, idempotently. |
...
| Simulation trigger | Non-production only | HR/admin advances simulation time via `POST /simulation/advance`. | Detects Draw keys whose scheduled time is crossed by the virtual clock advance and starts them idempotently through the same scheduler/workflow path. |
...
**Implementation (DRAW006)**: `SimulationController.Advance` evaluates each day in the advance range and checks whether the configured cut-off moment in `DrawSchedulerOptions.PolicyTimeZoneId` (e.g. `"Central European Standard Time"` for Prague) falls within `(oldNow, newNow]`. The cut-off is converted to UTC using `TimeZoneInfo.GetUtcOffset` so daylight-saving transitions are respected. For each crossed cut-off, the target parking date (`crossedDate + TargetDateOffsetDays`) is passed to `IDrawSchedulerService.TriggerDueDrawsAsync(date, tenantId: tenantId)`, scoping the trigger to the advancing tenant only — other tenants' Draws are not fired. The Dapr cron path calls the same method with `tenantId: null` to trigger all configured tenants. The scheduler service uses the same `TriggerDrawCommand` → `TriggerDrawHandler` → `IDrawWorkflowStarter` path as Dapr cron, ensuring idempotency through deterministic workflow instance IDs and Draw attempt acquisition. Duplicate advances or scheduler ticks remain safe because existing "Completed" or "InProgress" Draw attempts prevent re-execution.
