...
**Implementation (DRAW006)**: `SimulationController.Advance` now detects all UTC dates crossed during the time advance and calls `IDrawSchedulerService.TriggerDueDrawsAsync` for each crossed date's target parking date (computed as `crossedDate + TargetDateOffsetDays`). The scheduler service uses the same `TriggerDrawCommand` → `TriggerDrawHandler` → `IDrawWorkflowStarter` path as Dapr cron, ensuring idempotency through deterministic workflow instance IDs and Draw attempt acquisition. Duplicate advances or scheduler ticks remain safe because existing "Completed" or "InProgress" Draw attempts prevent re-execution.
