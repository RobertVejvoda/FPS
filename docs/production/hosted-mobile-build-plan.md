...
> The mobile OIDC redirect uses the app's custom scheme (`fpsmobile://…`), not a web URL, so no web redirect host is needed for the native flow. Register the mobile client's redirect URI in Keycloak as part of OPS012 (public-domain auth).
...
| Mobile OIDC client + redirect not registered for the public domain | TestFlight/internal hosted login needs the real issuer + `fpsmobile://` redirect in Keycloak | OPS012 public-domain auth (#316) |
