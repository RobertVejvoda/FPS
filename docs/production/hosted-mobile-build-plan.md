...
The Expo app carries stable **FairSpot** product identity (issue #748). Build/package identifiers use `fairspot` naming; native reverse-DNS identifiers use `fairspot` (never `fps`):

| Field | Value | Where |
|---|---|---|
| App display name | `FairSpot` | `app.json` `expo.name` |
| Expo slug | `fairspot-mobile` | `app.json` `expo.slug` |
| npm package name | `fairspot-mobile` | `package.json` `name` |
| Custom scheme | `fairspot` | `app.json` `expo.scheme` |
| iOS bundle identifier | `net.vejvoda.fairspot.mobile` | `app.json` `expo.ios.bundleIdentifier` |
| Android package | `net.vejvoda.fairspot.mobile` | `app.json` `expo.android.package` |

**OIDC redirect URI:** with scheme `fairspot`, `AuthSession.makeRedirectUri({ path: 'login-callback' })` resolves to **`fairspot://login-callback`** for native/standalone builds. That is the redirect URI to allow-list in Keycloak for the target build profile (OPS012); confirm the exact generated value from the build/runtime before adding it.

**Runtime config names are unchanged for compatibility:** the `FPS_MOBILE_*` / `EXPO_PUBLIC_*` env var names and the OIDC **client id** `fps-mobile` are runtime/Keycloak configuration values, deliberately kept as-is (renaming the Keycloak client is out of scope for MOB011). Only the app's build/package identity moved to `fairspot`.
...
> The mobile OIDC redirect uses Expo AuthSession with the app's custom scheme (`fairspot`), not a web URL. The current app code calls `AuthSession.makeRedirectUri({ path: 'login-callback' })`, which resolves to `fairspot://login-callback` for native/standalone builds and must be registered in Keycloak for the target build profile as part of OPS012 (public-domain auth). Confirm the exact generated URI from the build/runtime being tested before adding the allow-list entry.
...
- [x] **App identifiers** — `ios.bundleIdentifier` and `android.package` are set to `net.vejvoda.fairspot.mobile`, with slug `fairspot-mobile` and scheme `fairspot` (MOB011, see [App identity](#app-identity-mob011)).
...
|---|---|---|
| ~~`ios.bundleIdentifier` / `android.package` unset~~ | Resolved in MOB011 (#748): both set to `net.vejvoda.fairspot.mobile`. | Done — see [App identity](#app-identity-mob011). |
