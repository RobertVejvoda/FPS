...
| CUST003 Tenant Workspace And Lifecycle | Planned | [#204](https://github.com/RobertVejvoda/FPS/issues/204) | - | `implemented-by: claude` target | After `CUST001` | Create tenant workspace, lifecycle state, service provisioning metadata, and audit-safe tenant metadata. |
| CUST004 Tenant Identity Setup And First Admin | Planned | [#202](https://github.com/RobertVejvoda/FPS/issues/202) | - | `implemented-by: claude` target | After `CUST003` | Configure trusted issuer/audience/role mapping and first administrator or break-glass local account. |
| CUST005 Tenant Parking Bootstrap | Planned | [#206](https://github.com/RobertVejvoda/FPS/issues/206) | - | `implemented-by: claude` target | After `CUST003`/`CFG002` | Use Configuration APIs to set tenant default policy, first location, and initial slots/capacity. |
| CUST006 Tenant Employee/Profile Bootstrap | Planned | [#201](https://github.com/RobertVejvoda/FPS/issues/201) | - | `implemented-by: claude` target | After `CUST004`/`P002` | Load minimal pilot employee/profile facts while preserving SSO-first and file-import constraints. |
