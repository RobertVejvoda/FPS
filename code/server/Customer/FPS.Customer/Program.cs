...
// Wait for Dapr sidecar before hydrating. Guard is present so test runs (no DAPR_HTTP_PORT)
// and local dev without a sidecar skip the poll without stalling for 5 minutes.
...
// restarts the pod when Dapr is unavailable. This prevents the fail-open path in
// TenantClaimsTransformation (IsEnforcementActive==false passes raw roles when stores empty).
