...
**In effect**
- Roles follow the **invocation, not the agent name**. Codex is the default Product Owner (writes specs, validates) and is the reviewer when invoked via `@codex review` (`chatgpt-codex-connector`). Claude, Copilot, and Codex can each be invoked as an **Implementer** by assigning them an issue (`anthropic-code-agent` / `copilot-swe-agent` / `openai-code-agent` — the assignment opens a branch/PR). Per PR the invariant holds: **Reviewer ≠ Implementer ≠ Merger** — whoever implemented it must not review or merge it, so a Codex-authored PR is reviewed by Claude or a human, never by Codex.
