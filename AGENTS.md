...
**Who merges what.** The auto-merge gate merges only **Copilot-authored, low-risk** PRs; everything else is human-merged, by design:

- **Changes to the automation and guardrails themselves** — `.github/**`, the gate / review-handoff / auto-ready workflows, `tools/security-gate.sh`, `CODEOWNERS`, the agent-governance docs (`AGENTS.md`, `AGENT_COOPERATION.md`), and `copilot-setup-steps` — are HIGH_RISK, so the gate skips them and a **human merges**. A bot must never auto-merge a change to the workflows or docs that govern it (self-modification protection); this is why the pipeline PRs themselves are always merged by hand.
- **High-risk product paths** (auth/security/identity, infra/deploy, DB migrations, dependency/toolchain manifests, runtime/app config, fairness/draw core) go to a human via CODEOWNERS.
- **Claude- or human-authored PRs** are not Copilot-authored, so the gate skips them: they are reviewed (by Codex via `@codex review`, or by Claude / a human) and a **human merges**. The implementer of a PR never reviews or merges it — a Codex-authored PR (issue-assigned Codex) is reviewed by Claude or a human, not by Codex. This follows the per-invocation roles above (Reviewer ≠ Implementer ≠ Merger, per PR).

Net: assign routine, low-risk product work to Copilot and it merges hands-off (Codex reviews → the App bot merges); pipeline, infra, security, governance, and architecture changes — including any change to this delivery automation — always pass through a human.
